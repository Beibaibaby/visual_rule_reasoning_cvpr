...
class dataset(Dataset):
    def __init__(self, root, dataset_type, fig_type='*', img_size=160, transform=None, num_label_raven=None,
                 is_train=False, train_label=True):
        # dataset_type: 'train', 'val', 'test'
        self.transform = transform
        self.img_size = img_size
        self.file_names = [f for f in glob.glob(os.path.join(root, fig_type, '*.npz')) if dataset_type in f]

        if is_train and train_label:
            self.file_names = self.file_names[:num_label_raven]
        elif is_train and not train_label:
            self.file_names = self.file_names[num_label_raven:]
        else:
            pass
