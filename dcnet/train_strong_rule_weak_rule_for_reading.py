...
logfile="./expri/"+args.dataset+"/expri_nov3_30_number="+str(args.num_label_raven)+"_gpu="+args.gpu+"_train_strong_rule_weak_rule_for_reading.txt"
...
def train(epoch):
    ...
    for batch_idx in trange(len(train_loader_iter), ncols=ncols_const):
        ...
        if label_aug:###For labeled data, wo do weak picture aug
        ...
        ###For unlabeled data, wo do strong rule aug
        ...
        for stu_model_output in list_stu_model_output:
            #print(guass_label.shape)
            #print(stu_model_output.shape)
            #unlabel_loss += F.mse_loss(guass_label, stu_model_output)
            #print(F.mse_loss(guass_label, stu_model_output))
            
            guass_label_1=guass_label.cpu().detach().numpy()
            stu_model_output_1 = stu_model_output.cpu().detach().numpy()
        
            lll=0
            count=0
            for itr in range(stu_model_output_1.shape[0]):
                if np.max(np.absolute(stu_model_output_1[itr]))>thre:
                    count+=1
                    mse = (np.square(stu_model_output_1 - guass_label_1)).mean()
                    lll+=mse
            unlabel_loss += (lll/count)
            #image_tensor = torch.tensor(image_tensor)
