...
seq_strong_picture = iaa.Sequential([
...
def data_auger_strong_rule(image_tensor):
    image_tensor = image_tensor.numpy()
    image_tensor = seq_strong(images=image_tensor)
    image_tensor = torch.tensor(image_tensor)
    return image_tensor

def data_auger_strong_picture(image_tensor):
    image_tensor = image_tensor.numpy()
    image_tensor = seq_strong_picture(images=image_tensor)
    image_tensor = torch.tensor(image_tensor)
    return image_tensor
...
def strong_picture(image):
    for i in range(image.shape[-4]):
        image[i,:,:,:]= data_auger_strong_picture(image[i,:,:,:])
    return image
...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for batch_data in tqdm(self.trainloader, f'Train epoch {epoch}', ncols=ncols_const):
            ...
            guass_label_npy = label_guessed.cpu().detach().numpy()

            unlabel_loss = 0
            for stu_model_output in list_unlabel_output:
                stu_model_output_npy = stu_model_output[0].cpu().detach().numpy()
                lll=0
                count=0
                for itr in range(stu_model_output_npy.shape[0]):
                    if np.max(np.absolute(stu_model_output_npy[itr]))>self.args.thre:
                        count+=1
                        mse = (np.square(stu_model_output_npy - guass_label_npy)).mean()
                        lll+=mse
                    if count !=0:
